...
func (r *postRepo) Create(req *pb.PostInfo) (*pb.PostInfo, error) {
	result := pb.PostInfo{}
	err := r.db.QueryRow(`insert into posts (
	poster_id,
	description_post
	) values ($1, $2) returning poster_id,description_post`, req.PosterId, req.DescriptionPost).Scan(
		&result.PosterId,
	...
	for _, md := range req.Medias {
		...
		link,
		type) values ($1,$2,$3,$4)
		returning name, link, type`, md.PostId, md.Name, md.Link, md.Type).Scan(
...
func (r *postRepo) Update(req *pb.PostInfo) (*pb.PostInfo, error) {
	...
	name = $1,
    link = $2,
    type = $3 WHERE post_id = $4 returning id, name, link, type`, address.Name, address.Link, address.Type, req.Id).Scan(
			&adress.PostId,
			&adress.Name,
			&address.Link,
			&address.Type)
